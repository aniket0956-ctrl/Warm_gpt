...
Please join the channel to unlock AI features\\!"""
        ...
        keyboard.append([InlineKeyboardButton("✅ I've Joined the Channel", callback_data="check_membership")])
        
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        # FIXED: Handle both command and callback scenarios
        if update.message:
            ...
            # If called from button callback
            ...
            # Fallback: send directly using bot
    ...
    async def send_main_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        message = f"🎯 **Worm GPT NaNo12a AI**\n\nHello {update.effective_user.first_name}! How can I help you today?"
        ...
        if query.data == "check_membership":
            ...
            if is_member:
                ...
                await query.edit_message_text(
                    "✅ **Verification Successful!**\n\nWelcome to Worm GPT NaNo12a AI!",
                    parse_mode=ParseMode.MARKDOWN
                )
                await self.send_welcome_message(update, context)
            else:
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.db.is_user_verified(user_id):
            await update.message.reply_text(
                "❌ Please use /start to verify your membership first."
            )
            return
    ...
    async def send_streaming_response(self, update: Update, context: ContextTypes.DEFAULT_TYPE, full_response: str):
        ...
        try:
            ...
            message = await update.message.reply_text("🤖 *Thinking\\.\\.\\.*", parse_mode=ParseMode.MARKDOWN_V2)
            ...
            for i, word in enumerate(words):
                ...
                if (i + 1) % 5 == 0 or i == len(words) - 1:
                    try:
                        formatted_text = f"🤖 {current_text.strip()}"
    ...
    async def menu_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not self.db.is_user_verified(user_id):
            await update.message.reply_text("❌ Please use /start to verify your membership first.")
            return
    ...
    async def broadcast_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not users:
            await update.message.reply_text("❌ No verified users found for broadcasting.")
            return
...
3. Click "Chat with Worm GPT NaNo12a AI" to start AI conversation
